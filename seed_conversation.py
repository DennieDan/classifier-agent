...
def main() -> None:
    ...
    for m in messages:
        if type(m).__name__ == "HumanMessage":
            ...
            break
    name = prompt[:50] + ("..." if len(prompt) > 50 else "") if prompt else "Virgin Olive Oil (sample)"
    final_content = ""
    for m in reversed(messages):
        ...
        if content and "Final Answer:" in content:
            ...
            break
    permit_id = insert_conversation(name=name, prompt=prompt or "What is the HS-Code of Virgin Olive Oil?", response=final_content)
